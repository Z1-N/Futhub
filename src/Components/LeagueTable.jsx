...
const leagueColors = {
  'PL': 'from-purple-600 to-indigo-600', // Premier League
  'BL1': 'from-red-600 to-yellow-500',   // Bundesliga
  'SA': 'from-blue-700 to-teal-500',     // Serie A
  'PD': 'from-red-500 to-purple-600',    // La Liga
  'FL1': 'from-blue-600 to-yellow-400',  // Ligue 1
  'CL': 'from-blue-900 to-indigo-700',   // Champions League
};
...
const LeagueTable = ({ leagueId }) => {
  ...
  useEffect(() => {
    const fetchStandings = async () => {
      if (!currentLeagueId) return; // No league selected
      ...
      try {
        ...
        setStandings(response.data.standings[0]?.table || []); // Handle no data
        setLoading(false);
      } catch (error) {
  ...
  // Use leagueColors to dynamically apply the background color
